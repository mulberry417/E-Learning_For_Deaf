...
const Quiz = () => {
  ...
  const scoreHandler = input => {
    if (input == Questions[index].answer) {
      ToastAndroid.show('¡Sí! ¡Tu respuesta es correcta!', ToastAndroid.SHORT);
      setScore(score + 25);
    } else {
      ToastAndroid.show(
        `¡Uh oh! La respuesta correcta era ${Questions[index].answer}`,
        ToastAndroid.SHORT,
      );
    }
    if (index == 3) {
        ...
        <>
          <View style={styles.scoresContainer}>
            <Avatar
              ...
              rounded
              activeOpacity={0.4}
              onPress={() => console.warn('Abrir perfil')}
            />
            <Text style={styles.scoreText}>Tu puntuación es {score}</Text>
          </View>
          ...
          <View style={styles.buttonMainContainer}>
            ...
            <TouchableOpacity
              ...
              <Text style={styles.buttonText}> Estudiar otro tema</Text>
            </TouchableOpacity>
          </View>
        </>
      ) : (
